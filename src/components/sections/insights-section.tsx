...
export function InsightsSection() {
  ...
  return (
    <section className="section-tight bg-slate-50/70">
      ...
      {/* Cards — link to native /blog/[slug] pages for SEO crawl acceleration */}
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {featuredPosts.map((post) => (
            ...
            >
              ...
              {/* Title links to native page — Google crawls this from homepage */}
              <Link href={`/blog/${post.slug}`} className="group/title">
                <h3 className="insights-title mt-3 text-sm font-bold leading-snug text-slate-950 transition-colors duration-200 group-hover/title:text-red-700">
                  {post.title}
                </h3>
              </Link>
              ...
              <div className="insights-border mt-4 flex items-center justify-between gap-2 border-t border-slate-100 pt-4">
                ...
                {/* Primary CTA → native page (SEO value) */}
                <Link
                  href={`/blog/${post.slug}`}
